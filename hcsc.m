        ...
        for kT = 1:numel(T_list)
            ...
            if is_T01
                ...
                if isempty(idx_ud) || isempty(adata{idx_ud}) ...
                    ...
                    end_eff_ud  = min([end_i, numel(acc_ud_full), numel(vel_ud_full)]);

                    acc_ud = acc_ud_full(start_i:end_eff_ud);
                    vel_ud = vel_ud_full(start_i:end_eff_ud);

                    % 三分量合成
                    acc_3c = sqrt(acc_ew(1:numel(acc_ud)).^2 + ...
                                  acc_ns(1:numel(acc_ud)).^2 + ...
                                  acc_ud.^2);
                    vel_3c = sqrt(vel_ew(1:numel(vel_ud)).^2 + ...
                                  vel_ns(1:numel(vel_ud)).^2 + ...
                                  vel_ud.^2);
