...
try
    ...
    elseif contains(line, 'Dir.') && ~contains(line, 'Direction')
        ...
        if ~isempty(tokens)
            dir_raw = tokens{1}{1};
            % 判断是数字还是字母
            if all(isstrprop(dir_raw, 'digit'))
                % 转为数字
                d = str2double(dir_raw);
                % 映射表
                switch d
                    case 5
                        earthquake_data.direction = "E-W";
                    case 4
                        earthquake_data.direction = "N-S";
                    case 6
                        earthquake_data.direction = "U-D";
                    otherwise
                        earthquake_data.direction = "UNKNOWN";
                end
            else
                % 原样采用字母格式（E-W / N-S / U-D）
                earthquake_data.direction = string(dir_raw);
            end
        end
